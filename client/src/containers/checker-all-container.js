...
import { CheckersAll } from '../components/pages'
...
class Operators extends React.Component {
    ...
    componentDidMount() {
        ...
        history.push('/chackers/');
        this.getAssortedUsers();
    }

    getAssortedUsers = () => {
        this.setState({ loading: true, error: null })
        axiosInterceptors.get(`/api/user/all/?byRole=${CHACKER}`)
            .then((res) => {
    ...
    downloadUserXlsx = () => {
        axiosInterceptors.get('/api/xlsx/export/user', {
            params: {
                byRole: "checker"
            },
            responseType: 'blob'
    ...
    render() {
        return (
            <CheckersAll {...this.state} userId={this.props.match.params.id} downloadUserXlsx={this.downloadUserXlsx} />);
    }
}
